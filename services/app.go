...
func GetBestOfCategoryAndSubs(category string, page int, page_str int) []models.App {
  apps := GetBestOfCategory(category, page, page_str)
  subCats := GetSubCategories(category)

  for _, subCat := range(subCats) {
    subApps := GetBestOfCategoryAndSubs(subCat, page, page_str)
...
func GetBestOfCategory(category string, page int, per_page int) []models.App {
  ...
  for rows.Next() {
      ...
      publisher, _ := GetAccountByID(pid)
